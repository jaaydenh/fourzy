...
namespace Fourzy
{
    ...
    public class GamePlayManager : UnitySingleton<GamePlayManager> {
        ...
        GameObject SpawnPiece(float posX, float posY, PlayerEnum player, PieceAnimState startingState)
        {
            ...
            if (game.gameState.GameType == GameType.REALTIME) {
                if (UserManager.instance.gamePieceId == game.opponent.gamePieceId && player == PlayerEnum.TWO) {
                    gamePiece.GetComponent<GamePiece>().SetAlternateColor(true);
                } else {
                    gamePiece.GetComponent<GamePiece>().SetAlternateColor(false);
                }

                if (game.opponent.gamePieceId > GamePieceSelectionManager.Instance.gamePieces.Count - 1) {
                    game.opponent.gamePieceId = 0;
                }

                if (game.isCurrentPlayer_PlayerOne) {
                    if (player == PlayerEnum.ONE) {
                        gamePiece.GetComponent<SpriteRenderer>().sprite = GamePieceSelectionManager.Instance.GetGamePieceSprite(UserManager.instance.gamePieceId);
                    } else {
                        gamePiece.GetComponent<SpriteRenderer>().sprite = GamePieceSelectionManager.Instance.GetGamePieceSprite(game.opponent.gamePieceId);
                    }
                } else {
                    if (player == PlayerEnum.ONE) {
                        gamePiece.GetComponent<SpriteRenderer>().sprite = GamePieceSelectionManager.Instance.GetGamePieceSprite(game.opponent.gamePieceId);
