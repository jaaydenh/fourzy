...
namespace Fourzy._Updates.UI.Menu.Screens
{
    public class PauseMenuScreen : MenuScreen
    {
        ...
        public void ShowRules()
        {
            ...
            {
                case GameType.REALTIME:
                    secondaryPopup = menuController.GetOrAddScreen<PromptScreen>().Prompt(
                        LocalizationManager.Value("rules"), 
                        LocalizationManager.Value("rules_realtime_game"), 
                        "", 
                ...
                case GameType.PASSANDPLAY:
                    secondaryPopup = menuController.GetOrAddScreen<PromptScreen>().Prompt(
                        LocalizationManager.Value("rules"), 
                        LocalizationManager.Value("rules_passandplay_game"), 
                        "", 
                        LocalizationManager.Value("close"));
            ...
            switch (GamePlayManager.Instance.Game._Mode)
            {
                case GameMode.GAUNTLET:
                    secondaryPopup = menuController.GetOrAddScreen<PromptScreen>().Prompt(
                        LocalizationManager.Value("rules"), 
                        LocalizationManager.Value("rules_gauntlet_game"), 
                        "", 
                        LocalizationManager.Value("close"));
                ...
                case GameMode.PUZZLE_FAST:
                    secondaryPopup = menuController.GetOrAddScreen<PromptScreen>().Prompt(
                        LocalizationManager.Value("rules"), 
                        LocalizationManager.Value("rules_puzzlepack_game"), 
                        "",
                        LocalizationManager.Value("close"));
