    ...
    public class GameBoard
    {
        ...
        public bool didPlayerWin(int player)
        {
            for (var row = 0; row < numRows; row++)
            {
                ...
                {
                    if (squaresMatchPiece(player, col, row, 1, 0))
                    { // Check for win in a column
                        return true;
                    }
                    else if (squaresMatchPiece(player, col, row, 0, 1))
                    { //Check for win in a row
                        return true;
                    }
                    else if (squaresMatchPiece(player, col, row, 1, 1))
                    { // Check for win in diagonal from upper left to lower right
                        return true;
                    }
                    else if (squaresMatchPiece(player, col, row, 1, -1))
                    { // Check for win in diagonal from upper right to lower left
                        return true;
                    }
                }
            }
