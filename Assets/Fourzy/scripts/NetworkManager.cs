...
namespace ConnectFour
{
	public class NetworkManager : MonoBehaviour
	{
		...
		private const string CognitoIdentityPoolId = "us-east-1:a9022f3f-bda6-4626-87ae-1359f8cc9c8f";
		private const string MobileAnaylticsAppId = "03fcd0a2b90347f4832dde912aed62df";
		...
		private const string NewMatchLambdaFunctionName = "NewFourzyMatch";
		...
		private const string SelfIsBluePropertyName = "selfIsBlue";
		private const string BoardNotationPropertyName = "boardNotation";
		private const string MovesPropertyName = "moves";
		private const string RequesterIdPropertyName = "requesterId";
		private const string OpponentIdPropertyName = "opponentId";
		private const string BluePlayerDynamoDBIndexKey = "BluePlayerId-index";
		private const string RedPlayerDynamoDBIndexKey = "RedPlayerId-index";
			...
			using (Dataset matchesDataset = CognitoSyncManager.OpenOrCreateDataset(MatchesDatasetName))
			{
				foreach (var matchState in gameState.MatchStates.Values)
				{
					if (matchState.Opponent.IsLocalOpponent())
					{
						...
						writer.WritePropertyName(SelfIsBluePropertyName);
						writer.Write(matchState.SelfIsBlue);
						writer.WritePropertyName(BoardNotationPropertyName);
						writer.Write(matchState.BoardState.ToForsythEdwardsNotation());
						writer.WritePropertyName(MovesPropertyName);
		...
		private static Dictionary<string, GameState.MatchState> MatchesDatasetToDict(Dataset matchesDataset, Dictionary<string, GameState.PlayerInfo> friendsDict)
		{
			...
			foreach (string matchId in matchesStringDict.Keys)
			{
				...
				if (friendsDict.ContainsKey(friendId))
				{
					...
					if (data[BoardNotationPropertyName] != null && data[MovesPropertyName] != null && data[SelfIsBluePropertyName] != null)
					{
						boardNotation = (string)data[BoardNotationPropertyName];
						moves = (string)data[MovesPropertyName];
						selfIsBlue = (bool)data[SelfIsBluePropertyName];
					}
					matchesDict.Add(matchId, new GameState.MatchState(friend, boardNotation, moves, selfIsBlue, matchId));
				}
			}
			return matchesDict;
		}
		...
		// from the FourzyMatches DynamoDB table.
		[DynamoDBTable("FourzyMatches")]
		private class SimpleMatchInfo
		{
			...
			{
				bool selfIsBlue = selfId == BluePlayerId;
				string opponentId = selfIsBlue ? RedPlayerId : BluePlayerId;
				GameState.PlayerInfo opponent = GameManager.Instance.GetFriendIfKnown(opponentId);
				return new GameState.MatchState(opponent, MatchState, Moves, selfIsBlue, MatchId);
			}
		}
			...
			ThreadPool.QueueUserWorkItem(delegate
				{
					...
					// Query matches to the BluePlayerId 
					var asyncSearchBluePlayer = DynamoDBContext.QueryAsync<SimpleMatchInfo>(Credentials.GetCachedIdentityId(), new DynamoDBOperationConfig()
						{
							IndexName = BluePlayerDynamoDBIndexKey
						});
					// Query matches to the RedPlayerId 
					var asyncSearchRedPlayer = DynamoDBContext.QueryAsync<SimpleMatchInfo>(Credentials.GetCachedIdentityId(), new DynamoDBOperationConfig()
						{
							IndexName = RedPlayerDynamoDBIndexKey
						});
					// Get the Matches from the first query
					asyncSearchBluePlayer.GetRemainingAsync((responseBluePlayer) =>
						{
							if (responseBluePlayer.Exception == null)
							{
								List<SimpleMatchInfo> matchInfosBluePlayer = responseBluePlayer.Result;
								foreach (var matchInfo in matchInfosBluePlayer)
								{
									matchStates.Add(matchInfo.ToMatchState(selfId));
								}
								// Get the Matches from the second query
								asyncSearchRedPlayer.GetRemainingAsync((responseRedPlayer) =>
									{
										if (responseRedPlayer.Exception == null)
										{
											List<SimpleMatchInfo> matchInfosRedPlayer = responseRedPlayer.Result;
											foreach (var matchInfo in matchInfosRedPlayer)
											{
												matchStates.Add(matchInfo.ToMatchState(selfId));
											}
										...
										{
											callback(responseRedPlayer.Exception.Message, null);
										}
									});
							}
							else
							{
								callback(responseBluePlayer.Exception.Message, null);
							}
						});
				});
		}

		// Write the match to the FourzyMatches DynamoDB table
		public void SaveOnlineMatchAsync(GameState.MatchState matchState, SaveOnlineMatchResponseCallback callback)
		{
			...
			matchInfo.MatchState = matchState.BoardState.ToForsythEdwardsNotation();
			matchInfo.Moves = matchState.BoardState.PreviousMove.ToLongAlgebraicNotation();
			matchInfo.BluePlayerId = matchState.SelfIsBlue ? Credentials.GetCachedIdentityId() : matchState.Opponent.Id;
			matchInfo.RedPlayerId = matchState.SelfIsBlue ? matchState.Opponent.Id : Credentials.GetCachedIdentityId();
